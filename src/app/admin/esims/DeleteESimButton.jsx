...
import { deleteESim } from "@/lib/actions/admin/esims.actions";
import { toast } from "react-toastify";

export default function DeleteESimButton({ id, planName }) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const res = await deleteESim(id);
      if (res.success) {
        toast.success(`${planName} deleted successfully`);
      } else {
        toast.error(res.error || "Failed to delete eSIM plan");
      }
    } catch (err) {
