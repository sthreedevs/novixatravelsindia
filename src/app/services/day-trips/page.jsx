...
const DayTrips = () => {
  ...
  const [dayTripData, setDayTripData] = useState(null);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`/api/dayTrip/`);
      setDayTripData(response.data.data);
    } catch (error) {
  ...
  return (
    <div className="py-20">
      ...
      {modal && (
        <ModalForm location={location} tripData={modal} setModal={setModal} />
      )}
    </div>
  );
};

const ModalForm = ({ tripData, setModal, location }) => {
  ...
  const handleOfflinePayment = async () => {
    ...
    try {
      ...
      await axios.post("/api/service/dayTripEnquiry", formData, {
